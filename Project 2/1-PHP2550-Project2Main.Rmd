...
imputed_data <- mice(data, m = 5, method = "pmm", maxit = 50, seed = 2024)
```
...
fit_models <- function(data) {
  ...
  predictors <- colnames(data)[!colnames(data) %in% c("abst")]
  outcome <- data$abst
...
```{r fit-multiple-imputed, echo=TRUE}
...
combined_df %>%
  kable(row.names = F, 
        ...
        caption = "Summary of Coefficients and Odds Ratios for Potential Moderator Effects across Model Selection Methods") %>%
...
```{r, fig.width = 10, fig.height = 10, out.width="80%", fig.align='center'}
...
combined_df %>%
  kable(
    row.names = F,
    col.names = c("Variable", "Coef", "OR", "Coef", "OR", "Coef", "OR"),
    caption = "Summary of Coefficients and Odds Ratios for Potential Predictor Effects across Model Selection Methods"
  ) %>%
  add_header_above(c(
    " " = 1,
    "Stepwise" = 2,
